    ...
    init() {
      ...
      const initInstVars = {
        ...
        Text: (
          ...
          inst.halign = ["left", "center", "right"].findIndex(
            (x) => x === horizontalAlign
          ) * 50;
          inst.valign = ["top", "center", "bottom"].findIndex((x) => x === verticalAlign) *
            50;
        ...
        async applyCurrentLevel() {
          ...
          Object.keys(this.curLevel.layers).forEach((layer) => {
            ...
            let allInstances = Object.keys(this.curLevel.layers[layer])
              .map((type) =>
                this.curLevel.layers[layer][type].map((x) => ({
                  ...x,
                  type
                }))
              )
